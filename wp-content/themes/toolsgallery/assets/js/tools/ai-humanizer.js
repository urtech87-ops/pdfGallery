(function () {
  ...
  lbl1.textContent = 'AI-Generated Text';
  var textarea = document.createElement('textarea');
  textarea.className = 'tg-ai5-textarea';
  textarea.placeholder = 'Paste AI-generated text here...';
  var w1 = document.createElement('div'); w1.appendChild(lbl1); w1.appendChild(textarea);
  form.appendChild(w1);

  /* Level */
  var lbl2 = document.createElement('label');
  lbl2.className = 'tg-ai5-label';
  lbl2.textContent = 'Humanization Level';
  var levelGroup = makeSingleToggleGroup(['Subtle', 'Moderate', 'Strong'], 'Moderate');
  var w2 = document.createElement('div'); w2.appendChild(lbl2); w2.appendChild(levelGroup);
  form.appendChild(w2);

  /* Style */
  var lbl3 = document.createElement('label');
  lbl3.className = 'tg-ai5-label';
  lbl3.textContent = 'Writing Style';
  var styleGroup = makeSingleToggleGroup(['Conversational', 'Professional', 'Academic', 'Casual'], 'Conversational');
  var w3 = document.createElement('div'); w3.appendChild(lbl3); w3.appendChild(styleGroup);
  form.appendChild(w3);

  /* Add elements (multi-select toggles) */
  var lbl4 = document.createElement('label');
  lbl4.className = 'tg-ai5-label';
  lbl4.textContent = 'Add Elements';
  var addOptions = ['Personal anecdotes', 'Rhetorical questions', 'Natural imperfections', 'None'];
  var addWrap = document.createElement('div');
  addWrap.className = 'tg-ai5-toggles';
  var addBtns = {};
  addOptions.forEach(function (opt) {
    var btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'tg-ai5-toggle';
    btn.textContent = opt;
    btn.addEventListener('click', function () {
      if (opt === 'None') {
        /* deselect all others, toggle None */
        addOptions.forEach(function (o) {
          if (o !== 'None') addBtns[o].classList.remove('active');
        });
        btn.classList.toggle('active');
      } else {
        /* deselect None when another is chosen */
        addBtns['None'].classList.remove('active');
        btn.classList.toggle('active');
      }
    });
    addBtns[opt] = btn;
  ...
  var w4 = document.createElement('div'); w4.appendChild(lbl4); w4.appendChild(addWrap);
  form.appendChild(w4);
  ...
  submitBtn.textContent = 'Humanize Text';
  ...
  loading.innerHTML = '<span class="tg-ai5-dot"></span><span class="tg-ai5-dot"></span><span class="tg-ai5-dot"></span><span>Humanizing text…</span>';
  ...
  copyBtn.className = 'tg-ai5-action-btn'; copyBtn.textContent = 'Copy';
  var dlBtn = document.createElement('button');
  dlBtn.className = 'tg-ai5-action-btn'; dlBtn.textContent = 'Download';
  var tryBtn = document.createElement('button');
  tryBtn.className = 'tg-ai5-action-btn'; tryBtn.textContent = 'Try Again';
  ...
  /* ── Logic ── */
  textarea.addEventListener('input', function () { submitBtn.disabled = textarea.value.trim() === ''; });
  ...
  submitBtn.addEventListener('click', function () {
    ...
    var checked = [];
    addOptions.forEach(function (o) {
      if (addBtns[o].classList.contains('active')) checked.push(o);
    });
    var additions = checked.length ? checked.join(', ') : 'none';
    var payload = {
      text: text,
