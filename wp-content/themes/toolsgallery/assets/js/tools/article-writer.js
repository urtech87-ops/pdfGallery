(function () {
  ...
  function makeMultiToggleGroup(options, defaultActive) {
    var wrap = document.createElement('div');
    wrap.className = 'tg-ai5-toggles';
    options.forEach(function (opt) {
      var btn = document.createElement('button');
      btn.type = 'button';
      var isActive = defaultActive.indexOf(opt) !== -1;
      btn.className = 'tg-ai5-toggle' + (isActive ? ' active' : '');
      btn.textContent = opt;
    ...
    wrap.getValues = function () {
      return Array.from(wrap.querySelectorAll('.tg-ai5-toggle.active')).map(function (b) { return b.dataset.value; });
    };
    return wrap;
  }
  ...
  /* Topic */
  var lbl1 = document.createElement('label'); lbl1.className = 'tg-ai5-label'; lbl1.textContent = 'Article Title / Topic';
  var topicInput = document.createElement('input');
  topicInput.className = 'tg-ai5-input'; topicInput.type = 'text';
  topicInput.placeholder = 'Enter article title or topic...'; topicInput.required = true;
  var w1 = document.createElement('div'); w1.appendChild(lbl1); w1.appendChild(topicInput);
  form.appendChild(w1);

  /* Article type */
  var lbl2 = document.createElement('label'); lbl2.className = 'tg-ai5-label'; lbl2.textContent = 'Article Type';
  var typeSel = makeSelect(['Blog Post', 'News Article', 'How-To Guide', 'Listicle', 'Opinion Piece', 'Product Review'], 'Blog Post');
  var w2 = document.createElement('div'); w2.appendChild(lbl2); w2.appendChild(typeSel);
  form.appendChild(w2);

  /* Word count */
  var lbl3 = document.createElement('label'); lbl3.className = 'tg-ai5-label'; lbl3.textContent = 'Word Count';
  var wordGroup = makeSingleToggleGroup(['300', '500', '800', '1200', '1500', '2000'], '800');
  var w3 = document.createElement('div'); w3.appendChild(lbl3); w3.appendChild(wordGroup);
  form.appendChild(w3);

  /* Target audience */
  var lbl4 = document.createElement('label'); lbl4.className = 'tg-ai5-label'; lbl4.textContent = 'Target Audience';
  var audienceInput = document.createElement('input');
  audienceInput.className = 'tg-ai5-input'; audienceInput.type = 'text';
  audienceInput.placeholder = 'e.g. beginners in Python';
  var w4 = document.createElement('div'); w4.appendChild(lbl4); w4.appendChild(audienceInput);
  form.appendChild(w4);

  /* Tone */
  var lbl5 = document.createElement('label'); lbl5.className = 'tg-ai5-label'; lbl5.textContent = 'Tone';
  var toneGroup = makeSingleToggleGroup(['Informative', 'Conversational', 'Professional', 'Enthusiastic', 'Neutral'], 'Informative');
  var w5 = document.createElement('div'); w5.appendChild(lbl5); w5.appendChild(toneGroup);
  form.appendChild(w5);

  /* Include */
  var lbl6 = document.createElement('label'); lbl6.className = 'tg-ai5-label'; lbl6.textContent = 'Include';
  var includeGroup = makeMultiToggleGroup(
    ['Introduction', 'Subheadings', 'Conclusion', 'Key takeaways', 'FAQ section'],
    ['Introduction', 'Subheadings', 'Conclusion']
  );
  var w6 = document.createElement('div'); w6.appendChild(lbl6); w6.appendChild(includeGroup);
  form.appendChild(w6);

  /* Keywords */
  var lbl7 = document.createElement('label'); lbl7.className = 'tg-ai5-label'; lbl7.textContent = 'Keywords (optional)';
  var kwInput = document.createElement('input');
  kwInput.className = 'tg-ai5-input'; kwInput.type = 'text';
  kwInput.placeholder = 'comma-separated keywords';
  var w7 = document.createElement('div'); w7.appendChild(lbl7); w7.appendChild(kwInput);
  form.appendChild(w7);

  /* Submit */
  var submitBtn = document.createElement('button');
  submitBtn.type = 'button'; submitBtn.className = 'tg-ai5-btn';
  submitBtn.textContent = 'Write Article'; submitBtn.disabled = true;
  ...
  loading.innerHTML = '<span class="tg-ai5-dot"></span><span class="tg-ai5-dot"></span><span class="tg-ai5-dot"></span><span>Writing article…</span>';
  ...
  submitBtn.addEventListener('click', function () {
    ...
    var includes = includeGroup.getValues();
    var payload = {
      topic: topic,
      type: typeSel.value,
      ...
      includes: includes.length ? includes.join(', ') : 'none',
